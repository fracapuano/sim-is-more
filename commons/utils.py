...
def read_lookup_table(dataset:str="cifar100"):
    ...
    if dataset not in ["cifar10", "cifar100", "ImageNet16-120"]:
        if 'imagenet' not in dataset.lower():
            ...
            dataset = 'imagenet'
    lookup_table = np.loadtxt(str(get_project_root()) + "/archive/" + f'cachedmetrics/{dataset}_cachedmetrics.txt', skiprows=1)
    return lookup_table
...
def read_test_metrics(dataset:str="cifar100"):
    ...
    if dataset not in ["cifar10", "cifar100", "ImageNet16-120"]:
        if 'imagenet' not in dataset.lower():
            ...
            dataset = 'imagenet'
    lookup_table = np.loadtxt(str(get_project_root()) + "/archive/" + f'cachedmetrics/{dataset}_perfmetrics.txt', skiprows=1)
    return lookup_table
...
def load_normalization(dataset:str, normalization:str):
    ...
    return pd.read_csv(str(get_project_root()) + "/archive/" + f'cachedmetrics/{dataset}_{normalization}.csv')
