...
class FastIndividual(BaseIndividual): 
    def __init__(
        self,
        genotype:List[str],
...
class Genetic: 
    ...
    def mutate(self, 
               ...
               n_loci:int=1, 
               genes_prob:Tuple[None, List[float]]=None) -> BaseIndividual: 
    ...
    def recombine(self, individuals:Iterable[Individual], P_parent1:float=0.5) -> BaseIndividual: 
...
class Population: 
    ...
    def update_population(self, new_population:Iterable[BaseIndividual]): 
        """Overwrites current population with new one stored in `new_population`"""
        if all([isinstance(el, BaseIndividual) for el in new_population]):
    ...
    def update_oldest(self, candidate:BaseIndividual): 
    ...
    def update_worst_n(self, candidate:BaseIndividual, attribute:str="fitness", n:int=2): 
    ...
    if not fast_inds: 
        ...
        population = [
            Individual(net=net, 
                       genotype=genotype, 
                       index=index,
                       searchspace_interface=searchspace_interface
                       )
        ...
        genotypes = map(lambda arch_string: searchspace_interface.architecture_to_list(architecture_string=arch_string), 
                        architectures)
        population = [
            FastIndividual(genotype=genotype, 
                           index=index, 
                           genotype_to_idx=searchspace_interface.architecture_to_index)
