...
class RENASEnv(BaseNASEnv):
    """
    gym.Env for RENAS algorithm.
    Environment used to reproduce the findings presented in: https://arxiv.org/abs/1808.00193
    """
    def __init__(self, 
    ...
    def get_reward(self, mutant:BaseIndividual)->float:
        """
        Compute the reward associated to a single architecture, as per RENAS. Architectures are actions.
        Here, the reward is defined as $\tan(f_C \cdot \pi / 2 )$, with $f_C$ being the eval accuracy of `mutant`.
        """
        f_C = self.searchspace.list_to_accuracy(mutant.genotype)
        return np.tan(f_C * np.pi / 2)
    ...
    def step(self, action:NDArray)->Tuple[NDArray, float, bool, dict]: 
        ...
        if not self.fast_inds:
            reinforced_individual = Individual(net=None, genotype=None, index=None)
        ...
        if not terminated: 
            ...
            return (self.searchspace.encode_architecture(
                        self.searchspace.list_to_architecture(
                            self.fittest_individual.genotype
                            )),
                   self.get_reward(self.fittest_individual),
                   terminated,
                   info)
